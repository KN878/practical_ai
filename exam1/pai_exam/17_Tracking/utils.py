...
def img_aug(img, bboxes, max_scale = 0.2, max_saturation =0.05, max_exposure = 0.05):
	...
	img = np.array(Image.fromarray(img).resize((h_new,w_new), -1))
...
def detect(image_name, net, c_threshold=0.5, overlap_threshold=0.5):
    ...
    img = np.array(Image.fromarray(ori_img).resize((448,448), 3))
    img = img/255.0
    img -= np.array([0.485, 0.456, 0.406])
    img /= np.array([0.229, 0.224, 0.225])
    ...
    if len(nmsbboxes) != 0:
        ...
        for bbox in nmsbboxes:
            areas.append(areaOfRectangle(bbox[2]-bbox[0], bbox[3]-bbox[1]))
        idx = areas.index(min(areas))
        mainbox = nmsbboxes[idx]
